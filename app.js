...
// ---------------- CONFIG ----------------
const EXEC_URL = "https://script.google.com/macros/s/AKfycbzjBZNzUtPVRIo7PQT_kN5aoH3r0CksHdmmzZoDHRfydrnhSthm4cGDqsXeJEwPsmEV/exec";
...
function sortByDisplayPriority(items) {
  ...
  return items.slice().sort((a, b) => {
    ...
    if (bYear !== aYear) return by - aYear;
...
function getAvailableTabs(rows, parallels) {
  ...
  const baseParallelsExist = parallels.some(p => {
...
function renderSubsetBlocks(groups, vars) {
  ...
  return groups.map(group => {
    const parallels = getParallelsForSectionSubset(currentProductTab === "Base Parallels" ? "Base" : currentProductTab, group.subset);
